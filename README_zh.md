...
LumiUI 是一个独立的 Swift Package，提供可复用的 SwiftUI 设计令牌、组件、图表与主题系统。它不依赖 Lumi 主应用或其他本地 package，既支持 macOS，也支持 iOS。
...
```swift
dependencies: [
    .package(url: "https://github.com/CofficLab/LumiUI.git", from: "1.0.0")
]
```

或者在 Xcode 中选择 **File → Add Package Dependencies...**，输入仓库地址并添加 `LumiUI` 产品。
